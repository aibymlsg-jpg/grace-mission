...
Equip a small-to-mid-size NGO (10–80 staff, multi-donor, often field-based) with a **multi-agent AI assistant** running locally inside Clawix, so that the NGO can:

1. Compress the time spent on donor-facing paperwork (proposals, narrative reports, log-frames) by at least half.
2. Raise the quality and traceability of monitoring & evaluation (M&E) data without adding headcount.
3. Keep beneficiary data, safeguarding records, and donor-restricted information inside a defensible, auditable boundary.
...
| 3 | **No autonomous money movement** | No agent may execute a transfer, payment, donor refund, or trade. Agents may *prepare* a payment instruction; a named human signs off out-of-band. |
| 4 | **Outbound comms are draft-only** | Email, SMS, social posts, donor portal submissions are drafted into files. Sending requires a human click. |
...
| 6 | **Web access** | Limited to a domain allowlist (donor portals, OECD-DAC, ReliefWeb, government registries). Suspicious links are surfaced to a human, never auto-opened. |
...
| 8 | **Refusals carry through** | If a skill or agent declines a task (e.g., "draft a misleading donor narrative"), no other agent in the system may complete it on its behalf. |
...
| `donor-engagement` | Draft proposals, narrative + financial reports, donor research | Read, Write, Edit, WebSearch (allowlisted) | donor-proposal, grant-research, impact-report |
...
Agents talk to each other through **shared files**, not direct delegation. The Program Coordinator drops a brief into `briefs/`; the Donor Engagement agent picks it up. This keeps each agent's authority narrow and the audit trail clean.

### 3.2 The skill library

Skills are reference packages — encoded best practice the relevant agent reads *before* drafting.

1. **donor-proposal** — log-frame structure, Theory of Change, OECD-DAC alignment, common donor templates (FCDO, USAID, ECHO, GAC, SDC, BMZ, private foundations).
...
5. **impact-report** — narrative report structure by donor, financial reporting touchpoints, beneficiary story consent rules.
6. **grant-research** — donor scanning checklist, eligibility filters, deadline tracking format.
...
|--------|---------------------|----------------------|--------|
| Hours per donor proposal (drafting only) | 35 hrs | ≤ 17 hrs | Self-reported, comparing 3 proposals before/after |
| Narrative reports submitted on time | 70% | ≥ 95% | Donor portal submission logs |
...
**Phase 0 — sandbox (week 1):** install Clawix, mount the NGO workspace, copy agents + skills, dry-run the Donor Engagement agent on one historical proposal.

**Phase 1 — donor cycle (weeks 2–6):** put Donor Engagement and M&E agents into live use on the next proposal and the next narrative report. Program Coordinator runs in shadow mode (drafts only, no writes outside `drafts/`).
