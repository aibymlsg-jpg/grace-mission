...
│     ▼             ▼                ▼              ▼                        │
│  ┌─────────┐ ┌──────────┐ ┌──────────────┐ ┌────────────┐ ┌────────────┐   │
│  │ program │ │  donor   │ │  monitoring  │ │  comms     │ │   field    │   │
...
└────────────────────────────────────────────────────────────────────────────┘
        ...
        ▼                                                       ▼
   KoboToolbox MCP                                       Donor portal MCP
...
4. **Approve MCPs (optional).** When an agent first asks for KoboToolbox or a donor portal, the user gets a `request_access` prompt. Approve only what is needed.
...
6. **Dry run.** Ask the Donor Engagement agent to redraft a *finished* proposal as a smoke test. Inspect the diff. Inspect the audit log.
...
- Skills are reviewed quarterly. Donor templates change; safeguarding policies tighten; the skill files must keep pace.

## What this configuration explicitly does NOT do

- It does not auto-send emails, donor submissions, social posts, or SMS.
